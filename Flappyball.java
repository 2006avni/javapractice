...
public class Flappyball extends JPanel implements ActionListener, KeyListener 
{

    private int ballY = 300; // ball's Y position
    private int velocity = 0; // ball's velocity (gravity + flap)
    private boolean gameOver = false;

    private ArrayList<Rectangle> pipes; // List of pipes
    private Timer timer;
    private int score = 0; // Score variable
    private boolean passedPipe = false; // Track when the ball passes a pipe
    ...
    public void actionPerformed(ActionEvent e) 
	{
        if (!gameOver) 
		{
            ...
            for (Rectangle pipe : pipes) 
			{
                if (pipe.intersects(new Rectangle(100, ballY, 20, 20))) {
                    gameOver = true;
                    timer.stop();
                }
            }

            // Increase score when the ball passes a pipe
            ...
            // Reset passedPipe when the ball approaches the next pipe
